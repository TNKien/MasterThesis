function [] = f_PlotEEG_BrainNetwork(w_bounds,n_bounds, nch, ijw, w_scal, n_features, n_scal, cbtype)
% 
...
if nargin == 3                          %Number of channels
%   disp(nch);                          %Displaying number of nodes (activate it for testing purposes)
    ...
    return;
elseif nargin == 5                      %Previous + Matrix and the Scaling of Weights
...
elseif nargin == 8                      %Previous + Nodes' attributes and Node Scaling
    w_atribut = ijw(:, 3);              
    w_scaling = w_scal;
    ...
    warning('You must insert 3, or 5, or 8 inputs');
...
elseif nchannels == 19
    labels = labelsEEG{5};      %personnalized montage of 19 labels
else
...
RGBlinks = squeeze(double2rgb(w_atribut_new, colormap(jet), w_bounds));
% RGBlinks = squeeze(double2rgb(w_atribut_new, flipud(colormap(bone)) ));
% RGBlinks = squeeze(double2rgb(w_atribut_new, colormap(bone)) );
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % 
...
switch n_scaling
    ...
    case 'n_nn2nx'          % scaling between nn to nx (default nn=1, nx=10)
        ...
        n_atribut_new = n_atribut ;
        n_atribut_new(8) = mean(n_atribut_new(n_atribut_new > 0)) ;
        % % % associating nodes' attributes to colormap JET
        RGBnodes = squeeze(double2rgb(n_atribut_new, colormap(jet),n_bounds));
    otherwise
...
%h = figure('Visible','off');%set(h, 'Visible', 'off'); % dont display the figure
...
for lk = 1 : size(ijw, 1)           %along all links
    ...
    if nargin < 8 
        line(xynodes(:, 1), xynodes(:, 2), 'LineWidth', w_atribut_new(lk), 'Color', RGBlinks(lk, :)); end
    % % %     line(xynodes(:, 1), xynodes(:, 2), 'LineWidth', ijw(lk, 3), 'Color', [.8 .8 .1]); % links de un solo color
    
    for nd = 1 : 2                                  %for the two nodes of a link
        r = 1.5 + ((n_atribut_new(ijw(lk,nd)) - n_bounds(1))*2 / (n_bounds(2)-n_bounds(1)));
        if nargin == 5 
            r = 1 ; end
        plot(xynodes(nd, 1), xynodes(nd, 2), 'o', 'MarkerSize', 5*pi.*r, ...
